  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6">
        {isLoading && roles.length === 0 ? <Spinner /> : (
                ...
                <table className="w-full">
                    ...
                    {roles.map((role) => (
                        ...
                        <td className="p-4 flex justify-center space-x-2">
                            <button onClick={() => openModal('edit', role)} className="btn btn-3d-pastel bg-brand-warning text-yellow-800 p-2.5"><FaEdit /></button>
                            <button onClick={() => setDeleteConfirm({ isOpen: true, roleId: role._id, roleName: role.name })} className="btn btn-3d-pastel bg-brand-danger text-red-800 p-2.5"><FaTrash /></button>
                        </td>
                        </tr>
                    ))}
                    </tbody>
         ...
         <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col transform transition-all duration-300">
            ...
            <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto pr-2 space-y-6">
                <div>
                    ...
                    <input type="text" value={roleName} onChange={(e) => setRoleName(e.target.value)} className="form-input" placeholder="เช่น พนักงานแคชเชียร์"/>
                </div>
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-xl">
                        {ALL_PERMISSIONS.map((permission) => (
                            <label key={permission.id} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-white cursor-pointer">
                                <input type="checkbox" className="form-checkbox h-5 w-5 text-brand-purple rounded-md border-gray-300 focus:ring-brand-purple" checked={selectedPermissions.includes(permission.id)} onChange={() => handlePermissionChange(permission.id)}/>
            ...
            <div className="flex items-center justify-end mt-8 pt-6 border-t border-gray-200">
               <button type="button" onClick={closeModal} className="btn bg-gray-200 text-gray-700 hover:bg-gray-300 mr-4">ยกเลิก</button>
               <button onClick={handleSubmit} className="btn btn-3d-pastel btn-primary">บันทึก</button>
             </div>
         </div>
       </div>
      )}
      ...
      {deleteConfirm.isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50 animate-fade-in">
            <div className="bg-white p-8 rounded-2xl shadow-xl text-center">
                ...
                <p className="text-gray-600 my-4">คุณแน่ใจหรือไม่ว่าต้องการลบตำแหน่ง <strong className='text-red-500'>{deleteConfirm.roleName}</strong>? <br/>การกระทำนี้ไม่สามารถย้อนกลับได้</p>
                <div className="flex justify-center space-x-4">
                    <button onClick={() => setDeleteConfirm({ isOpen: false, roleId: null, roleName: '' })} className="btn bg-gray-200 hover:bg-gray-300">ยกเลิก</button>
                    <button onClick={() => handleDelete(deleteConfirm.roleId)} className="btn btn-3d-pastel btn-danger">ยืนยันการลบ</button>
                </div>
            </div>
        </div>
      )}
