...
const SubMenu = ({ item, isSidebarOpen, setIsSidebarOpen, userPermissions }) => {
  ...
  return (
    <>
      ...
      {isSubmenuOpen && isSidebarOpen && ( <div className="ml-4 border-l-2 border-brand-purple-light/20">{availableSubmenu.map((subItem, index) => (<NavLink key={index} to={subItem.path} className={getSubLinkClass}>{subItem.title}</NavLink>))}</div> )}
    </>
  );
};
...
function Sidebar({ isSidebarOpen, setIsSidebarOpen }) {
    ...
    return (
      <>
        <aside onClick={handleSidebarClick} className={`bg-brand-sidebar border-r border-gray-200/80 transition-all duration-300 ease-in-out flex flex-col h-screen ${isSidebarOpen ? "w-64" : "w-20 cursor-pointer"}`}>
